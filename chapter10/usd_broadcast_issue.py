...
class MiniAsset(names.UsdAsset):
    drop = ('code', 'media', 'area', 'stream', 'step', 'variant')
...
def main():
    ...
    with Sdf.ChangeBlock():
        ...
        for unit in geom_plane, model_default_color:
    ...
    with cook.taxonomy_context(stage):
        ...
        for taxon in (place,):  # base xformable taxa
    ...
    romania, inherits_country, specializes_country, unchanged, *__ = cook.create_many(country, ('Romania', 'Inherits', 'Specializes', 'Unchanged'))
    ...
    golden_asset_name = MiniAsset(Usd.ModelAPI(golden_krone).GetAssetIdentifier().path)
    ...
    romania_asset_name = MiniAsset(Usd.ModelAPI(romania).GetAssetIdentifier().path)
