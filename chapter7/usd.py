...
def main():
    ...
    pr.enable()
    amount = 2_000
    pr.runcall(write.create_many, city, (f'NewCity{x}' for x in range(amount)), (f'New City Hello {x}' for x in range(amount)))
    pr.disable()
    pr.dump_stats(str(Path(__file__).parent / "stats_create_many.log"))
    end = datetime.datetime.now()
    print(f"Total time: {end - start}")
    # write.create_many(city, (f'NewCity{x}' for x in range(amount)), (f'New City Hello {x}' for x in range(amount)))
